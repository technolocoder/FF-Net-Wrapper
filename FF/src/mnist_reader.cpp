...
TEMPLATE void mnist_reader<T>::init(unsigned int sample_size ,char *image_path, char *label_path){
    ...
    data = (unsigned char*)malloc(785*sample_size+784*sample_size*sizeof(T)*2+10*sample_size*sizeof(T));
    raw_image = data;
    ...
    processed_image_normalized = (T*)(data+785*sample_size+784*sample_size*sizeof(T)+10*sample_size*sizeof(T));
    ...
    for(int i = 0; i < sample_size; ++i) {
        for(int j = 0; j < 784; ++j){
            processed_image[i*784+j] = raw_image[i*784+j]/255.0;
            processed_image_normalized[i*784+j] = processed_image[i*784+j]*2.0-1.0;
        }
...
TEMPLATE T *mnist_reader<T>::get_processed_image_mean_normalized(){
